<div class="flex justify-center mt-8 px-0 sm:items-center sm:justify-between">
    <div class="text-center text-sm text-gray-500 sm:text-left">
        <div class="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="-mt-px mr-1 w-5 h-5 stroke-gray-400 group-hover:stroke-gray-600">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
            </svg>
            Made by
            <a href="https://github.com/kublahanov" class="group px-1 inline-flex items-center hover:text-red-700 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">
                kublahanov
            </a>
        </div>
    </div>
    <div class="ml-4 text-center text-sm text-gray-500 sm:text-right sm:ml-0">
        Powered by:
        <span class="text-red-500">Laravel</span> v{{ Illuminate\Foundation\Application::VERSION }}
        (<span class="text-blue-500">PHP</span> v{{ PHP_VERSION }})
    </div>
</div>
